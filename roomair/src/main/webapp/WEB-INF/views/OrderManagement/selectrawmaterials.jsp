...
<body>
	<div class="content">
		...
		<input id="rCInput" type="hidden"> <input id="rNInput" type="hidden"> <input id="rTInput" type="hidden"> <input id="rPInput" type="hidden"> <input id="wCInput" type="hidden"> <input id="wNInput" type="hidden"> <input id="sCInput" type="hidden"> <input id="rUInput" type="hidden">
    ...
    const newWindow = window.open(url, '_blank', popupFeatures); 
		...
		<form action="${pageContext.request.contextPath}/OrderManagement/selectrawmaterials" method="get" id="searchBox">
			<div id="searchForm" style="border-radius: 5px;">
				<label>원자재코드</label> <input type="text" name="search1" placeholder="원자재코드"> <label>원자재명</label> <input type="text" name="search2" placeholder="원자재명"> <label>종류</label> <select name="search3">
				...
				</select> <label>거래처</label> <input type="text" name="search4" placeholder="거래처" onclick="openPopup3()"> <input type="submit" value="조회" id="searchButton">
			</div>
		</form>
		<br>
