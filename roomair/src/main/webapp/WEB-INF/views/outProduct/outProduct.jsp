...
<link href="${pageContext.request.contextPath }/resources/css/outProduct.css" rel="stylesheet" type="text/css">
...
<body>
	...
	<div id="con">
		<h2>
			<a href="${pageContext.request.contextPath}/outProduct/list" style="text-decoration: none; color: black;">출고 관리</a>
		</h2>
		<!-- 		<hr> -->
		<div id="searchForm">
			<label>출고 코드 </label><input type="text" name="outCode" id="outCode"> <label>제품명 </label><input type="text" name="prodName" id="prodName9999" placeholder="제품명" onclick="searchItem('prod','prodCode9999')" readonly="readonly"> <label>거래처명 </label><input type="text" name="clientCompany" id="sellclientCompany9999" placeholder="거래처명" onclick="searchItem('sellclient','sellclientCode9999')" readonly="readonly"> <input type="button" value="조회" id="searchButton"> <input type="button" value="초기화" id="resetButton">
		</div>
		<!-- 		<hr> -->
		<div id="buttons">
			<input type="button" class="buttons highlighted" value="전체" id="allButton"> <input type="button" class="buttons " value="미출고" id="non_deliveryButton"> <input type="button" class="buttons " value="중간납품" id="interim_deliveryButton"> <input type="button" class="buttons " value="출고완료" id="deliveryButton">
	...
	<script type="text/javascript">
		...
		$(document).ready(
				function() {
					...
					$("#outCode, #prodName9999, #clientCompany9999").on(
							'keydown', function(e) {
								if (e.key === 'Enter') {
		...
		function updateOutProductList(data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (i < data.length - 1) {
					...
					var row = $("<tr>");
					row.append("<td>"
							+ (data[i].outCode ? data[i].outCode : '-')
							+ "</td>");
					row.append("<td>"
							+ (data[i].sellCode ? data[i].sellCode : '-')
							+ "</td>");
					row.append("<td onclick='getInfo(event, \""
					...
					row.append("<td onclick='getInfo(event, \""
							+ (data[i].prodCode ? data[i].prodCode : '-')
							+ "\")' style='cursor:pointer;'>"
							+ (data[i].prodCode ? data[i].prodCode : '-')
							+ "</td>");
					...
					row.append("<td>"
							+ (data[i].sellCount ? data[i].sellCount : '-')
							+ "</td>");
					row.append("<td>"
							+ (data[i].outCount ? data[i].outCount : '-')
							+ "</td>");
					row.append("<td>"
					...
					row
							.append("<td>"
									+ (data[i].outPrice ? formatCurrency(data[i].outPrice)
											: '-') + "</td>");
					row.append("<td>"
							+ (data[i].sellDuedate ? data[i].sellDuedate : '-')
							+ "</td>");
					row.append("<td>"
							+ (data[i].outDate ? data[i].outDate : '-')
							+ "</td>");
					row.append("<td>"
							+ (data[i].outEmpId ? data[i].outEmpId : '-')
							+ "</td>");
					row.append("<td>"
							+ (data[i].sellState ? data[i].sellState : '-')
							+ "</td>");
						...
						button
								.click(function() {
									...
									window
											.open(
													contextPath
															+ "/outProduct/outProductContent?outCode="
															+ outCode,
													"출고 상세정보",
													"width=500,height=730,toolbar=no,location=no,resizable=yes");
								});
											...
											var modifiedData = data
													.map(function(item) {
														return {
															...
															'남은 납품 개수' : item.sellCount
																	- item.outCount,
															'출고 개수' : item.outCount,
															'재고 개수' : item.stockCount,
															'납품 단가' : item.prodPrice,
															'현재 출고가' : item.prodPrice
																	* item.outCount,
															'비고' : item.outMemo
														};
													});
		...
		function openModalWithData(event, data, width) {
			...
			if (event.clientX + window.scrollX + width >= bodyWidth) {
				modal.style.left = (event.clientX + window.scrollX - width)
						+ 'px';
			} else {
