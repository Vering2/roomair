...
<body>
	<c:if test="${!(empty sessionScope.empDepartment) && (sessionScope.empDepartment eq '관리자' || sessionScope.empDepartment eq '자재팀')}">
		...
		<form action="${pageContext.request.contextPath}/outProduct/outProductUpdate" id="updateForm" method="POST">
				...
				<tbody>
					...
					<tr>
						...
						<c:if test="${outProductDTO.outEmpId == null}">
							<td><input type="hidden" name="outEmpId" value="${sessionScope.empId}" id="outEmpId" readonly="readonly"> <input type="text" name="outEmpId_1" value="${sessionScope.empId}" id="outEmpId_1" readonly="readonly" style="cursor: pointer;"></td>
						</c:if>
						<c:if test="${outProductDTO.outEmpId != null  }">
							<td><input type="hidden" name="outEmpId" value="${sessionScope.empId}" id="outEmpId" readonly="readonly"> <input type="text" name="outEmpId_2" value="${outProductDTO.outEmpId}" id="outEmpId_1" readonly="readonly" style="cursor: pointer;"></td>
						</c:if>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="hidden" name="outPrice" value="${outProductDTO.outPrice }"> <input type="text" name="outPriceFormat" value="${outPrice }원" readonly="readonly"></td>
					</tr>
				</tbody>
			</table>
			<br>
			<table class="outProductTable">
				...
				<tr>
					<td colspan="3"><input type="hidden" id="initialOutCount" value="${outProductDTO.outCount}"> <c:if test="${outProductDTO.stockCount == null || outProductDTO.stockCount == 0}">
							<input type="number" name="outCount" value="${outProductDTO.outCount}" id="inputNum" readonly="readonly">
						</c:if> <c:if test="${outProductDTO.stockCount != null && outProductDTO.stockCount > 0}">
					...
					<td colspan="3"><input type="hidden" id="initialstockCount" value="${outProductDTO.stockCount}"> <input type="number" name="stockCount" value="${outProductDTO.stockCount }" min="0" readonly="readonly"></td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="3"><fmt:formatNumber var="prodPrice" value="${outProductDTO.prodPrice }" pattern="###,###.##"></fmt:formatNumber> <input type="hidden" name="prodPrice" value="${outProductDTO.prodPrice }"> <input type="text" name="prodPriceFormat" value="${prodPrice }원" readonly="readonly"></td>
					<td colspan="3"><input type="text" name="currentOutPrice" value="" readonly="readonly"></td>
				</tr>
				<tr>
	...
	<script type="text/javascript">
		function updateInventory() {
			...
			var outCountInput = document
					.querySelector('input[name="outCount"]');
			...
			var prodPrice = $
			{
				outProductDTO.prodPrice
			}
			;
			// 현재 출고가
			var currentOutPrice = document
					.querySelector('input[name="currentOutPrice"]');

			// 현재 출력해야되는 재고값 계산
			var initialstockCount = parseInt(document
					.getElementById('initialstockCount').value, 10);
			var initialOutCount = parseInt(document
					.getElementById('initialOutCount').value, 10);
			var outCount = parseInt(outCountInput.value, 10);

			// 재고 입력란 업데이트
			stockCountInput.value = initialstockCount + initialOutCount
					- outCount;
		...
		$(document)
				.ready(
						function() {
							...
							var stockCount = $
							{
								outProductDTO.stockCount
							}
							; // stockCount 값을 JSP 표현식으로 가져옴
							var sellCount = $
							{
								outProductDTO.sellCount
							}
							; // sellCount 값을 JSP 표현식으로 가져옴
							var outCount = $
							{
								outProductDTO.outCount
							}
							;

							// maxCount 값을 설정할 때 Math.min 함수를 사용하여 더 작은 값을 선택
							var maxCount = Math.min(sellCount, outCount
									+ stockCount);
