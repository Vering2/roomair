		...
		<c:when test="${!(empty sessionScope.empDepartment)}">
			<div class="container">
				...
				<form action="${pageContext.request.contextPath}/product/writePro" method="post">
					...
					<div class="form-group">
						<p>용량(ml)</p>
						<input type="text" name="prodSize" placeholder="용량을 입력해 주세요." oninput="this.value = this.value.replace(/[^0-9]/g, '')">
						<!--     정수 숫자만 입력 -->
					</div>
					<div class="form-group">
						<p>향기</p>
					...
					<div class="form-group">
						<p>거래처명</p>
						<input type="hidden" name="clientCode" id="sellclientCode9999" onclick="searchItem('sellclient','sellclientCode9999')"> <input type="text" name="clientCompany" id="sellclientCompany9999" readonly onclick="searchItem('sellclient','sellclientCode9999')" placeholder="거래처를 선택해 주세요." style="cursor: pointer;">
					</div>
					<div class="form-group">
						<p>창고명</p>
						<input type="hidden" name="whseCode" id="whseCode9999" onclick="searchItem('whse','whseCode9999')"> <input type="text" name="whseName" id="whseName9999" placeholder="창고를 선택해 주세요." onclick="searchItem('whse','whseCode9999')" style="cursor: pointer;" readonly>
					</div>
					<div class="form-group">
						<p>매출 단가(원)</p>
						<input type="number" step="0.01" name="prodPrice" placeholder="매출 단가를 입력해 주세요." oninput="this.value = this.value.replace(/[^0-9.]/g, ''); if(this.value.indexOf('.') !== -1){if(this.value.split('.')[1].length > 2){this.value = this.value.slice(0, -1)}};">
						<!--     소수점아래 두자리수까지 숫자입력 -->
					</div>
					<p class="memo">비고</p>
					<textarea name="prodMemo" class="prodMemo" placeholder="비고를 입력해 주세요." rows="5" cols=""></textarea>
					<div id="button">
						<input type="submit" value="확인"> <input type="button" value="닫기" onclick="closeWindow()">
					</div>
				</form>
			</div>
									...
									.click(
											function() {
														...
														.ajax({
															...
															success : function(
																...
																console
																		.log(
																				"Ajax 내부에서의 codeNum:",
																				codeNum); // Ajax 내부에서의 codeNum: [받아온 데이터]
												...
												function someFunction(data) {
													...
													var num = parseInt(codeNum
															.substring(2))
															+ counter + 1; // 문자열을 숫자로 변환하여 1 증가
													var paddedNum = padNumber(
															num,
															codeNum.length - 2); // 숫자를 패딩하여 길이 유지
													prodCode = codeNum
															.charAt(0)
															+ codeNum.charAt(1)
															+ paddedNum
																	.toString(); // 패딩된 숫자를 다시 문자열로 변환
													counter++;
												} // someFunction(data)

												document
														.getElementById("prodCode").textContent = prodCode;
											});
