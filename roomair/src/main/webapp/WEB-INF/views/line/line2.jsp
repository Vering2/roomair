...
<script type="text/javascript">
	document.addEventListener('DOMContentLoaded', function() {
		document.querySelector('.lineProcess').addEventListener('change',
				function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$.each(data, function(key, entry) {
				if (entry.empId != 1) {
					var optionText = '사원번호=' + entry.empId + ', 사원이름='
							+ entry.empName;
					dropdown.append($('<option></option>').attr('value',
							entry.empId).text(optionText));
				}
			});
		}
	});
	...
	<div class="container">
		...
		<form action="${pageContext.request.contextPath}/line/insertPro" id="join" method="post">
			...
			<div class="form-group">
				<p>라인코드:</p>
				<input type="text" name="lineCode" class="lineCode" value="L10">
			</div>
			<div class="form-group">
				<p>라인명:</p>
			...
			<div class="form-group">
				<p>등록자:</p>
				<select name="lineEmpId" class="lineEmpId select"></select>
			</div>
			<div class="form-group">
				<p>등록일:</p>
				<input type="date" name="lineInsertDate" class="lineInsertDate">
			</div>
			<div class="form-group">
				<p>공정:</p>
