		...
		if (isPop) {
			...
			$('table tr').click(
					function() {
						...
						if (number != null) {
							$('#sellclientCompany' + number, opener.document)
									.val(clientCompany);
							$('#sellclientCode' + number, opener.document).val(
									clientCode);
						} else {
							$('#sellclientCompany', opener.document).val(
									clientCompany);
							$('#sellclientCode', opener.document).val(
									clientCode);
						}
	...
	<div id="pagination" class="page_wrap">
		<div class="page_nation" style="text-align: center; font-size: 0; position: absolute; bottom: 1%; right: 1%;">
			<c:if test="${paging.startPage != 1 }">
				<a class="arrow prev" href="${pageContext.request.contextPath}/search/sellclient?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}&clientCode=${dto.clientCode }&clientCompany=${dto.clientCompany }">◀️</a>
			</c:if>
			...
			<c:if test="${paging.endPage != paging.lastPage}">
				<a class="arrow next" href="${pageContext.request.contextPath}/search/sellclient?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}&clientCode=${dto.clientCode }&clientCompany=${dto.clientCompany }">▶️</a>
			</c:if>
		</div>
	</div>
</body>
