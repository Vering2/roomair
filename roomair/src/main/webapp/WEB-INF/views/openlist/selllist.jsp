			...
			$('table tr')
					.click(
							function() {
								...
								var clientCompany = $(this).find(
										'#clientCompany').text();
								var sellCode = $(this).find('#sellCode').text();
								var prodName = $(this).find('#prodName').text();
								var prodCode = $(this).find('#prodCode').text();
								var workAmount = $(this).find('#workAmount')
										.text();
								var sellCount = $(this).find('#sellCount')
										.text();

								var number = isPop.match(/\d+/);
								if (number != null) {
									$('#sellCode' + number, opener.document)
											.val(sellCode);
									$('#clientCompany' + number,
											opener.document).val(clientCompany);
									$('#prodName' + number, opener.document)
											.val(prodName);
									$('#prodCode' + number, opener.document)
											.val(prodCode);
									$('#workAmount' + number, opener.document)
											.val(workAmount);
									$('#sellCount' + number, opener.document)
											.val(sellCount);

								} else {
									$('#sellCode8888', opener.document).val(
											sellCode);
									$('#clientCompany8888', opener.document)
											.val(clientCompany);
									$('#workAmount8888', opener.document).val(
											workAmount);
									$('#sellCount8888', opener.document).val(
											sellCount);
								}
	...
	<div class="right_col">
		...
		<form method="get">
			<fieldset>
				...
				<input type="hidden" name="input" id="input" value="${input }"> <label>코드&nbsp;</label> <input style="width: 175; font: 500 15px/15px 'Inter', sans-serif;" class="input_box" type="text" name="sellCode" id="searchCode" onfocus="this.value='SL'" placeholder="코드를 입력하세요."> &nbsp;&nbsp; <label>거래처&nbsp;</label> <input style="width: 175; font: 500 15px/15px 'Inter', sans-serif;" class="input_box" type="text" name="clientCompany" id="searchName" placeholder="거래처를 입력하세요."> &nbsp;&nbsp; <input type="submit" class="button" value="조회">
				...
				<div class="x_content">
					<div class="table-responsive">
						<div class="table-wrapper">
							...
							<table id="sellTable" class="table table-striped jambo_table bulk_action" style="text-align-last: center;">
								<thead>
									<tr class="headings">
										<th>번호</th>
										<th>코드</th>
