...
<body>
	<div class="perfdetail">
		...
		<form action="${pageContext.request.contextPath}/perf/perfinsertPro" id="perfinsert" method="POST">
			<table>
				<tbody>
					...
					<tr>
						<th>라인코드</th>
						<!-- worklist에서 받아옴 -->
						<td><input type="text" id="lineCode9999" name="lineCode" readonly></td>
						<!--  작업지시코드 -->
					</tr>

					<tr>
						<th>제품코드</th>
						<td><input type="text" id="prodCode9999" name="prodCode" readonly></td>
						<!--  제품코드 -->
					</tr>

					<tr>
						<th>실적일</th>
						<td><input type="text" id="perfDate" name="perfDate" readonly></td>
						<!-- 실적일자(자동생성) -->
					</tr>
					<tr>
						...
						<th>담당자</th>
						<!--  세션으로 넘겨받음 -->
						<td><input type="text" id="perfEmpId" name="perfEmpId" readonly></td>
						<!--  담당자아이디(세션으로처리) -->
					</tr>
					</tbdoy>
			</table>
			...
			<div class="footerbtn">
				<input type="submit" id="btn" class="btn" value="등록"> <input type="button" id="closebtn" class="btn" value="닫기">
			</div>
		</form>
	</div>
	...
	<script>
		...
		$(document).ready(
				function() {
					...
					var currentDate = new Date();
					var pfCode = 'PF' + currentDate.getFullYear()
		...
		document
				...
				.addEventListener(
						...
						function(event) {
							...
							$
									.post(
											"${pageContext.request.contextPath}/perfajax/ajaxinsert",
											$("#perfinsert").serialize(),
											function(response) {
												// 서버 응답에 대한 처리 코드
												if (response.success) {
