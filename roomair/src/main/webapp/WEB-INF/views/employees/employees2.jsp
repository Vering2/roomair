...
<body>
	<div class="container">
		...
		<form action="${pageContext.request.contextPath}/employees/insertPro" id="join" method="post" enctype="multipart/form-data">
			...
			<input type="text" id="empId" name="empId" class="empId" style="display: none;"> <input type="password" name="empPass" class="empPass" style="display: none;">
			<div class="form-group">
				<p>사원명</p>
				<input type="text" id="empName" name="empName" class="empName"> <br>
			</div>
			...
			<div class="form-group">
				<p>이메일</p>
				<input type="email" id="empEmail" name="empEmail" class="empEmail">
			</div>
			<span id="empEmailmsg"> </span>
			<div class="form-group">
				<p>전화번호</p>
				<input type="text" id="empTel" name="empTel" class="empTel" placeholder="- 없이 입력하세요" maxlength=11>
			</div>
			<span id="empTelmsg"> </span>
			<div class="form-group">
				<p>입사일자</p>
				<input type="date" name="empHiredate" class="empHiredate" required>
			</div>
			<div class="form-group">
				<p>재직구분</p>
			...
			<div class="form-group">
				...
				<!-- 사진 삭제 버튼 추가 -->
				<input type="file" id="file" name="file" accept="image/*" onchange="previewImage()" style="display: none;" /> <input type="hidden" id="oldfile" name="oldfile" value="${employeesDTO.empFile}">
			</div>
			<div class="form-group">
				<p></p>
				<c:choose>
	...
	<script>
		...
		document
				.getElementById("empEmail")
				.addEventListener(
						"input",
						function() {
							var empEmail = this.value;

							empEmail = empEmail.replace(/[ㄱ-ㅎ가-힣]/g, '')
									.replace(/[^a-zA-Z0-9.@]/g, '');
		...
		form
				.addEventListener(
						...
						function(event) {
							...
							empTel = empTel.replace(/-/g, '');
							empTel = empTel.replace(/[^\d]/g, '');
							if (!phoneNumberRegex.test(empTel)
									|| empTel.length !== 11) {
								document.getElementById("empTelmsg").textContent = "올바른 휴대폰 번호를 입력하세요.";
								document.getElementById("empTelmsg").style.position = "relative"; // 메시지 표시
								event.preventDefault(); // 폼 제출을 막습니다.
